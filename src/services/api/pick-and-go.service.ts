...
export class PickAndGoApiService extends BaseApiService {
  ...
  async createOrder(orderData: CreateOrderRequest) {
    try {
      ...
      const response = await this.api.post<{ success: boolean; data: PickAndGoOrder }>(
        `${this.baseUrl}/orders`,
        orderData
      );
  ...
  async getOrder(orderId: string) {
    try {
      ...
      const response = await this.api.get<{ success: boolean; data: PickAndGoOrder }>(
        `${this.baseUrl}/orders/${orderId}`
      );

      if (response.data.success) {
        console.log('✅ Order retrieved successfully with', response.data.data.items?.length || 0, 'items');
        return { success: true, data: response.data.data };
      } else {
  ...
  async getUserOrders(userId: string, filters?: {
    ...
    try {
      ...
      const response = await this.api.get<{ success: boolean; data: PickAndGoOrder[] }>(url);

      if (response.data.success) {
        console.log('✅ Retrieved', response.data.data.length, 'orders for user');
        return { success: true, data: response.data.data };
      } else {
  ...
  async addItemToOrder(orderId: string, itemData: AddItemRequest) {
    try {
      ...
      const response = await this.api.post<{ success: boolean; data: PickAndGoItem }>(
   ...
   */
  async updateOrderStatus(orderId: string, orderStatus: string, prepMetadata?: Record<string, any>) {
    try {
      console.log('🔄 Updating order status:', orderId, 'to', orderStatus);

      const response = await this.api.put<{ success: boolean; data: PickAndGoOrder }>(
   ...
   * @returns Promise con la orden actualizada
   */
  async updatePaymentStatus(orderId: string, paymentStatus: 'pending' | 'paid') {
    try {
   ...
   */
  async estimatePrepTime(items: Array<{ item: string; quantity: number }>, restaurantId?: number) {
    try {
      ...
      if (response.data.success) {
        console.log('✅ Estimated prep time:', response.data.data.estimated_minutes, 'minutes');
        return { success: true, data: response.data.data };
      } else {
   ...
   */
  async getRestaurantOrders(restaurantId: number, filters?: {
    order_status?: string;
    date_from?: string;
    date_to?: string;
  }) {
    try {
      ...
      if (response.data.success) {
        console.log('✅ Retrieved', response.data.data.length, 'restaurant orders');
        return { success: true, data: response.data.data };
      } else {
