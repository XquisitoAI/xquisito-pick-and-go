...
export default function PhoneCapturePage() {
  ...
  // Skip - go directly to payment-success
  const handleSkip = () => {
    navigateToSuccess();
  };
  ...
  const handleVerifyOTP = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.success && response.data) {
        const verifiedUserId = response.data.user?.id;
        ...
        if (profileResponse.success && profileResponse.data) {
          ...
          // If profile has firstName, link order and go to success
          if (profile.firstName) {
            await refreshProfile();
            // Link order to customer using user.id from verifyOTP response
            if (orderId && verifiedUserId) {
              await pickAndGoService.linkOrderToCustomer(
                orderId,
                phone,
                verifiedUserId,
              );
  ...
  const handleProfileSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        // Link order to customer using userId from state (saved in handleVerifyOTP)
        if (orderId && userId) {
          await pickAndGoService.linkOrderToCustomer(orderId, phone, userId);
        }
      ...
      <button
        onClick={() => {
          if (step === "verify") {
            ...
            // From phone step, go to payment-success (skip)
            handleSkip();
          }
        }}
      ...
      <div className="relative z-10 w-full max-w-md">
        ...
        <div className="mb-8 text-center">
          ...
          <h1 className="text-2xl font-medium text-white">
            {step === "phone"
              ? "¿Quieres recibir el estatus del pedido?"
              : step === "verify"
                ? "Verifica tu código"
                : "Completa tu perfil"}
          </h1>
          <p className="text-gray-200 mt-2">
            {step === "phone"
              ? "Ingresa tu número para darte seguimiento"
              : step === "verify"
        ...
        {step === "phone" && (
          <form onSubmit={handleSendOTP} className="space-y-4">
            ...
            <button
              type="button"
              onClick={handleSkip}
              className="w-full text-white/80 hover:text-white py-3 text-sm underline transition-colors"
            >
              No gracias
            </button>
          </form>
